...
func (c *Cmd) GinPost(ctx *gin.Context) {
	...
	if c.genIUCtxF == nil {
		c.errF(ctx, gerr.NewErr(http.StatusInternalServerError, "internal server error"))
		return
	}
	...
	if c.IsUserAuthType() {
		if c.checkAuthF == nil {
			c.errF(ctx, gerr.NewErr(http.StatusUnauthorized, "unauthorized"))
			return
		}
	...
	// 走到这里说明走不动了
	c.errF(ctx, gerr.NewErr(http.StatusInternalServerError, "Internal error"))
}
