...
func (g *GormEngine) GetModelList(ctx uctx.IUCtx, req *gormx.GetModelListReq) (*gormx.GetModelListRsp, error) {
	...
	if !req.SkipCount && req.Offset == 0 && req.Limit > 0 {
		...
		if uint32(len(res.rows)) >= req.Limit {
			...
			if len(res.rows) == 0 {
				return nil, gerr.NewErr(-1, "empty response")
			}
			row := res.rows[0]
			if len(row) == 0 {
				return nil, gerr.NewErr(-1, "empty row")
			}
			if req.Group != "" {
...
// Exec 执行SQL 无返回结果
func (g *GormEngine) Exec(sql string) (err error) {
	db := g.GetDB("")
	return db.Exec(sql).Error
}

func (g *GormEngine) CheckTableExist(table string) (bool, error) {
	db := g.GetDB("")
	var tables []string
	err := db.Raw("SHOW TABLES LIKE ?", table).Scan(&tables).Error
	if err != nil {
		return false, err
	}
