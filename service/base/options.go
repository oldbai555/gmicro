...
func (p *OptionsProcessor) Process() error {
	...
	for _, v := range p.listOption.Options {
		...
		case OptValueTypeUint32:
			...
			if err != nil {
				return gerr.NewInvalidArg("invalid option value with type %d, expected uint32", v.Key)
			}
			if h.cbUint32 != nil {
		...
		case OptValueTypeUint64:
			...
			if err != nil {
				return gerr.NewInvalidArg("invalid option value with type %d, expected uint64", v.Key)
			}
			if h.cbUint64 != nil {
				err = h.cbUint64(x)
				if err != nil {
		...
		case OptValueTypeTimeStampRange:
			...
			if err != nil {
				return gerr.NewInvalidArg("invalid option value with type %d, expected begin_time_stamp-end_time_stamp", v.Key)
			}
		...
		case OptValueTypeInt32:
			...
			x, err := strconv.ParseInt(v.Value, 10, 32)
			if err != nil {
				return gerr.NewInvalidArg("invalid option value with type %d, expected int32", v.Key)
			}
			if h.cbInt32 != nil {
